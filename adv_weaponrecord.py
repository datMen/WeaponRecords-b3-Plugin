...
#--------------------------------------------------------------------------------------------------
class Adv_WeaponrecordPlugin(b3.plugin.Plugin):
    _adminPlugin = None
    _cronTab = None
    _msg = None
    _fileName = None
    ...
    def onStartup(self):
        ...
        self._weaponrecordPlugin = self.console.getPlugin('weaponrecord')
    ...
    def adv(self, firstTry=True):
        ...
        if ad:
            ...
            elif "@weapontopstats" in ad:
                ad2 = ad.split(" ")
                if self._weaponrecordPlugin:
                    data = ('%s' % ad2[1])
                    self._weaponrecordPlugin.cmd_weapontopstats(data, client=None, cmd=None, ext=True)
                    if firstTry:
                        # try another ad
                        self.adv(firstTry=False)
                        return
                    ...
                    self.error('Weaponrecord not installed! Cannot use @weapontopstats in adv plugin!')
                    ad = '@weapontopstats not available, Weaponrecord Plugin is not installed!'
            elif "@weaponmaprecord" in ad:
                ad2 = ad.split(" ")
                if self._weaponrecordPlugin:
                    data = ('%s' % ad2[1])
                    self._weaponrecordPlugin.cmd_weaponmaprecord(data, client=None, cmd=None, ext=True)
                    if firstTry:
                        # try another ad
                        self.adv(firstTry=False)
                        return
                    ...
                    self.error('Weaponrecord not installed! Cannot use @weaponmaprecord in adv plugin!')
                    ad = '@weaponmaprecord not available, Weaponrecord Plugin is not installed!'
            elif ad == "@admins":
                try:
